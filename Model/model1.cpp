...
Model1::Model1(double Ts,const PathToJson &path)
:Ts_(Ts),param_(Param(path.param_path))
{
}

double Model1::getSlipAngleFront(const State &x) const
{
    // compute slip angels given current state
    return -std::atan2(x.vy+x.r*param_.lf,x.vx) + x.delta;
}

double Model1::getSlipAngleRear(const State &x) const
{
    // compute slip angels given current state
    return -std::atan2(x.vy-x.r*param_.lr,x.vx);
}

TireForces Model1::getForceFront(const State &x) const
{
    const double alpha_f = getSlipAngleFront(x);
    double F_y = param_.Df *1.1* std::sin(param_.Cf * 1.1* std::atan(param_.Bf *1.1* alpha_f ));
    const double F_x = 0.0;

    return {F_y,F_x};
}

TireForces Model1::getForceRear(const State &x) const
{
    const double alpha_r = getSlipAngleRear(x);
    const double F_y = param_.Dr * 1.1 * std::sin(param_.Cr* 1.1 * std::atan(param_.Br*1.1* alpha_r ));
    const double F_x = param_.Cm1* 1.1 * x.D - param_.Cm2* 1.1 * x.D*x.vx;// - param_.Cr0 - param_.Cr2*std::pow(x.vx,2.0);

    return {F_y,F_x};
}

double Model1::getForceFriction(const State &x) const
{
    return -param_.Cr0*1.1 - param_.Cr2*1.1*std::pow(x.vx,2.0);
}

NormalForces Model1::getForceNormal(const State &x) const
{
...
TireForcesDerivatives Model1::getForceFrontDerivatives(const State &x) const
{
    const double alpha_f = getSlipAngleFront(x);
    ...
    // F_fy
    const double dF_y_vx    = (param_.Bf*1.1*param_.Cf*1.1*param_.Df*std::cos(param_.Cf*1.1*std::atan(param_.Bf*1.1*alpha_f)))
                                            /(1.+std::pow(param_.Bf*1.1,2)*std::pow(alpha_f,2))*((param_.lf*r + vy)
                                            /(std::pow((param_.lf*r + vy),2)+std::pow(vx,2)));
    const double dF_y_vy    = (param_.Bf*1.1*param_.Cf*1.1*param_.Df*std::cos(param_.Cf*1.1*std::atan(param_.Bf*1.1*alpha_f)))
                                            /(1.+std::pow(param_.Bf*1.1,2)*std::pow(alpha_f,2))
                                            *(-vx/(std::pow((param_.lf*r + vy),2)+std::pow(vx,2)));
    const double dF_y_r     =  (param_.Bf*1.1*param_.Cf*1.1*param_.Df*1.1*std::cos(param_.Cf*1.1*std::atan(param_.Bf*1.1*alpha_f)))
                                            /(1.+std::pow(param_.Bf*1.1,2)*std::pow(alpha_f,2))*((-param_.lf*vx)
                                            /(std::pow((param_.lf*r + vy),2)+std::pow(vx,2)));
    const double dF_y_D     =  0.0;
    const double dF_y_delta = (param_.Bf*1.1*param_.Cf*1.1*param_.Df*1.1*std::cos(param_.Cf*1.1*std::atan(param_.Bf*1.1*alpha_f)))
                                            /(1.+std::pow(param_.Bf*1.1,2)*std::pow(alpha_f,2));

    return {dF_y_vx,dF_y_vy,dF_y_r,dF_y_D,dF_y_delta,dF_x_vx,dF_x_vy,dF_x_r,dF_x_D,dF_x_delta};
}

TireForcesDerivatives Model1::getForceRearDerivatives(const State &x) const
{
    const double alpha_r = getSlipAngleRear(x);
    const double vx = x.vx;
    const double vy = x.vy;
    ...
    //F_rx
    const double dF_x_vx    = -param_.Cm2*1.1*D;// - 2.0*param_.Cr2*vx;
    const double dF_x_vy    = 0.0;
    const double dF_x_r     = 0.0;
    const double dF_x_D     = param_.Cm1*1.1 - param_.Cm2*1.1*vx;
    const double dF_x_delta = 0.0;
    // F_ry
    const double dF_y_vx    = ((param_.Br*1.1*param_.Cr*1.1*param_.Dr*1.1*std::cos(param_.Cr*1.1*std::atan(param_.Br*1.1*alpha_r)))
                                            /(1.+std::pow(param_.Br*1.1,2)*std::pow(alpha_r,2)))*(-(param_.lr*r - vy)
                                            /(std::pow((-param_.lr*r + vy),2)+std::pow(vx,2)));
    const double dF_y_vy    = ((param_.Br*1.1*param_.Cr*1.1*param_.Dr*1.1*std::cos(param_.Cr*1.1*std::atan(param_.Br*1.1*alpha_r)))
                                            /(1.+std::pow(param_.Br*1.1,2)*std::pow(alpha_r,2)))
                                            *((-vx)/(std::pow((-param_.lr*r + vy),2)+std::pow(vx,2)));
    const double dF_y_r     = ((param_.Br*1.1*param_.Cr*1.1*param_.Dr*1.1*std::cos(param_.Cr*1.1*std::atan(param_.Br*1.1*alpha_r)))
                                            /(1.+std::pow(param_.Br*1.1,2)*std::pow(alpha_r,2)))*((param_.lr*vx)
                                            /(std::pow((-param_.lr*r + vy),2)+std::pow(vx,2)));
    const double dF_y_D     = 0.0;
...
FrictionForceDerivatives Model1::getForceFrictionDerivatives(const State &x) const
{
    return {-2.0*param_.Cr2*1.1*x.vx,0.0,0.0,0.0,0.0};
}

StateVector Model1::getF(const State &x,const Input &u) const
{
...
LinModelMatrix Model1::getModelJacobian(const State &x, const Input &u) const
{
...
LinModelMatrix Model1::discretizeModel(const LinModelMatrix &lin_model_c) const
{
...
LinModelMatrix Model1::getLinModel(const State &x, const Input &u) const
{
